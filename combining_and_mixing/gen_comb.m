...
for k = 1:length(matches{n})
    if isempty(fxyc{look(n,o)}(matches{n}(k)).frame), continue; end
...
for k = 1:length(matches{swap(n)})
    if isempty(fxyc{o}(matches{swap(n)}(k)).frame), continue; end
...
while true
    ...
    for k = 1:length(matches{n})
        ...
        if isempty(fxyc{which_o(which_o~=o)}(matches{n}(k)).frame), continue; end
