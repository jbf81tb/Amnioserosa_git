...
%%
vec = -.15:.01:.15;
[bay,bax] = histcounts(nonzeros(cell2mat({nsta(basal).sl}')),vec);
bax = (bax(1:end-1)+bax(2:end))/2;
[apy,apx] = histcounts(nonzeros(cell2mat({nsta(apical).sl}')),vec);
apx = (apx(1:end-1)+apx(2:end))/2;
[oy,ox] = histcounts(nonzeros(cell2mat({nsta(~basal&~apical).sl}')),vec);
ox = (ox(1:end-1)+ox(2:end))/2;
[by,bx] = histcounts(nonzeros(cell2mat({nsta(good).sl}')),vec);
bx = (bx(1:end-1)+bx(2:end))/2;
[nby,nbx] = histcounts(nonzeros(cell2mat({nsta(~good).sl}')),vec);
